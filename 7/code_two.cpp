...
std::size_t count_bags(std::size_t to_color,
                       std::map<std::size_t, std::vector<Edge>>& edge_by_color) {
    std::size_t bags{0};
    for (auto& i : edge_by_color[to_color]) {
        if (i.bags() > 0) {
            bags += i.bags() + i.bags() * count_bags(i.to(), edge_by_color);
        }
    }
    return bags;
}
...
int main(int argc, char* argv[]) {
    ...
    auto bags = count_bags(get_color_id(Color("shiny gold"), nodes), graph_edge_by_color);

    std::cout << bags << " are required inside one shiny gold bag!\n";
